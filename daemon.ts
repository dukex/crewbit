...
import { loadConfig, createProvider, resolveNextAction } from './src/workflow.js'
...
function runClaude(action: QueueAction, config: WorkflowConfig, dryRun: boolean): void {
  ...
  log(`[${action.type.toUpperCase()}] ${action.issueKey} → worktree: ${worktreeName}`)

  if (dryRun) {
    log(`[dry-run] would run: claude -w "${worktreeName}" --print '${ralphLoop}'`)
    return
  }

  cleanWorktrees(config.daemon.worktreePrefix)

  const maxSeconds = Number(process.env.MAX_SESSION_SECONDS ?? config.daemon.maxSessionSeconds)
...
async function main(): Promise<void> {
  ...
  while (true) {
    try {
      ...
      const waitSeconds = Number(process.env.WAIT_SECONDS ?? config.daemon.waitSeconds)
